...
Check:
```{r}
anova(lm(totalcost ~ comp_bin + ERvisits + duration, data = heart_data_trans))
```
...
Judging from the p-value, we can see that at least one $\beta$ is not zero, so we reject the null. 
...
```{r}
anova(lm(satisfaction ~ age + severity, data = sat_data), lm(satisfaction ~ age + severity + anxiety, data = sat_data))
```
